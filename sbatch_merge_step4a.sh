#!/bin/bash -l
#SBATCH --job-name=merge_step4a
#SBATCH --partition=cpu
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=02:00:00
#SBATCH --output=logs/merge_step4a_%j.out
#SBATCH --error=logs/merge_step4a_%j.err
...
# Usage:
#   ARRAY_ID=$(sbatch --parsable sbatch_step4a_array.sh)
#   sbatch --dependency=afterok:${ARRAY_ID} sbatch_merge_step4a.sh ${ARRAY_ID}
#
# Ou en une commande :
#   ARRAY_ID=$(sbatch --parsable sbatch_step4a_array.sh) && \
#   sbatch --dependency=afterok:${ARRAY_ID} sbatch_merge_step4a.sh ${ARRAY_ID}
...
if [ -z "$ARRAY_JOB_ID" ]; then
    echo "[ERROR] Passer le ARRAY_JOB_ID en argument: sbatch sbatch_merge_step4a.sh <ARRAY_JOB_ID>"
    exit 1
fi

OUTDIR="${WORKDIR}/data/output"
OUTBASE="${OUTDIR}/step4a"
MERGED_PARQUET="${OUTDIR}/step4a_merged_job${ARRAY_JOB_ID}.parquet"
MERGED_CSV="${OUTDIR}/step4a_merged_job${ARRAY_JOB_ID}.csv"

echo "=== MERGE step4a array job ${ARRAY_JOB_ID} ==="
echo "DATE=$(date -Is)"
...
RUN_DIR="${WORKDIR}/data/output/step4a_merged_job${ARRAY_JOB_ID}"
mkdir -p "$RUN_DIR"

cp "$MERGED_CSV"                    "${RUN_DIR}/results.csv"            || true
cp "$MERGED_PARQUET"                "${RUN_DIR}/results.parquet"        || true
cp "src/step4a_prompts.py"               "${RUN_DIR}/prompts_used.py"        || true
cp "sbatch_step4a_array.sh"       "${RUN_DIR}/sbatch_array_used.sh"   || true
...
echo "Merge termine."
echo "${RUN_DIR}" > "${WORKDIR}/data/output/.last_step4a_archive"

# -- Auto-chain -----------------------------------------------------------------
sbatch "${WORKDIR}/sbatch_step4b_array.sh" "${MERGED_PARQUET}"
echo "[chain] -> sbatch_step4b_array.sh submitted (input: ${MERGED_PARQUET})"
