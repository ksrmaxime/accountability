#!/bin/bash -l
#SBATCH --job-name=step6_array
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=48G
#SBATCH --time=3-00:00:00
#SBATCH --output=logs/step6_array_%A_%a.out
#SBATCH --error=logs/step6_array_%A_%a.err
...
INPUT=${1:-"${WORKDIR}/data/output/step5_merged/results.parquet"}
OUTPUT_BASE=${WORKDIR}/data/output/step6
TEXT_COL=text
...
# Inference
BATCH_SIZE=4
MAX_NEW_TOKENS=150      # YES/NO + une phrase de justification
...
if [[ "${SLURM_ARRAY_TASK_ID:-}" == "0" ]]; then
    sbatch --dependency=afterok:${SLURM_ARRAY_JOB_ID} \
        "${WORKDIR}/sbatch_merge_step6.sh" "${SLURM_ARRAY_JOB_ID}"
    echo "[chain] Submitted sbatch_merge_step6.sh (dependency: afterok:${SLURM_ARRAY_JOB_ID})"
fi
