#!/bin/bash -l
#SBATCH --job-name=step5_array
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=48G
#SBATCH --time=3-00:00:00
#SBATCH --output=logs/step5_array_%A_%a.out
#SBATCH --error=logs/step5_array_%A_%a.err
...
INPUT=${1:-"${WORKDIR}/data/output/step4b_merged/results.parquet"}
OUTPUT_BASE=${WORKDIR}/data/output/step5
TEXT_COL=text
...
if [[ "${SLURM_ARRAY_TASK_ID:-}" == "0" ]]; then
    sbatch --dependency=afterok:${SLURM_ARRAY_JOB_ID} \
        "${WORKDIR}/sbatch_merge_step5.sh" "${SLURM_ARRAY_JOB_ID}"
    echo "[chain] Submitted sbatch_merge_step5.sh (dependency: afterok:${SLURM_ARRAY_JOB_ID})"
fi
